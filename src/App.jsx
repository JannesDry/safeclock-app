...
import { 
  ...
  Camera, LogIn, LogOut, Settings, Bell, AlertTriangle, Clock4, Shield, MapPin, Database, Cloud
} from 'lucide-react';
...
// FIX: STATIC SHARED APP ID
const appId = 'safeclock_production_v1';

// --- DATABASE LOCKED SCREEN ---
const DatabaseLocked = () => (
  <div className="flex flex-col items-center justify-center min-h-[50vh] p-8 text-center bg-white rounded-xl shadow-lg border-l-4 border-amber-500 max-w-2xl mx-auto mt-10">
    ...
    <p className="text-slate-600 mb-6 max-w-md">Your app is connected, but Firebase is blocking access. You need to enable "Public Mode" in your database rules.</p>
    <div className="bg-slate-50 p-4 rounded text-xs text-left font-mono">
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 font-sans text-slate-900 flex flex-col relative" style={{ fontFamily: 'system-ui, sans-serif', backgroundColor: '#f1f5f9', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      {showAdminLogin && (
        <div className="absolute inset-0 z-[100] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
            ...
            <input type="password" inputMode="numeric" placeholder="PIN" autoFocus className="w-full text-center text-3xl font-bold border-2 rounded-xl py-3 mb-4" value={adminPinInput} onChange={e => setAdminPinInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && verifyAdminPin()}/>
            {adminError && <div className="text-center text-red-500 text-sm mb-4">{adminError}</div>}
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setShowAdminLogin(false)} className="py-2 rounded-xl bg-slate-100">Cancel</button>
              <button onClick={verifyAdminPin} className="py-2 rounded-xl bg-indigo-600 text-white">Unlock</button>
            </div>
          </div>
        </div>
      )}
      ...
      <nav className="bg-slate-900 text-white p-4 shadow-md sticky top-0 z-50">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          {/* --- VISUAL VERSION MARKER --- */}
          <div className="flex items-center gap-2 font-bold text-lg">
            <Cloud className="text-green-400" /> SafeClock V2 (SHARED)
          </div>
          <div className="flex bg-slate-800 rounded-lg p-1">
            <button onClick={() => { if(viewMode!=='manager') { setShowAdminLogin(true); setAdminError(''); setAdminPinInput(''); } }} className={`px-4 py-1.5 rounded-md text-sm font-medium flex items-center gap-2 ${viewMode === 'manager' ? 'bg-indigo-600' : 'text-slate-400'}`}>Manager</button>
            <button onClick={() => setViewMode('kiosk')} className={`px-4 py-1.5 rounded-md text-sm font-medium flex items-center gap-2 ${viewMode === 'kiosk' ? 'bg-indigo-600' : 'text-slate-400'}`}>Kiosk</button>
          </div>
        </div>
      </nav>
...
function ManagerDashboard({ userId, kioskLocation, setKioskLocation }) {
  ...
  useEffect(() => {
    ...
    // SHARED PATHS (public/data)
    const unsubSettings = onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'config'), (doc) => {
        if (doc.exists()) { setSettings(prev => ({...prev, ...doc.data()})); if(doc.data().defaultShift) setNewEmpShift(doc.data().defaultShift); }
      }, handleError);
  ...
  const addEmployee = async (e) => {
    ...
    try {
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'employees'), {
        name: newEmpName, phoneNumber: newEmpPhone, shiftStart: newEmpShift, isKeyholder, currentCode: '-----', codeValidDate: '', lastClockIn: null, status: 'out'
      });
      setNewEmpName(''); setNewEmpPhone(''); setIsKeyholder(false);
  ...
  const isLate = (timestamp, shiftTime) => {
    ...
    try {
      const timeStr = timestamp.toDate().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false });
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      <div className="flex bg-slate-200 p-1 rounded-lg self-start overflow-x-auto w-fit">
        {['status', 'overview', 'reports', 'settings'].map(tab => (
           <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 rounded-md text-sm font-medium capitalize ${activeTab === tab ? 'bg-white text-indigo-600' : 'text-slate-600'}`}>{tab === 'status' ? 'Who is In?' : tab}</button>
        ))}
      </div>
          ...
          {employees.map(emp => (
            <div key={emp.id} className={`p-4 rounded-xl border flex items-center justify-between ${emp.status === 'in' ? 'bg-green-50 border-green-200' : 'bg-white opacity-70'}`}>
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${emp.status === 'in' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-400'}`}>{emp.name.charAt(0)}</div>
                <div><h3 className="font-bold">{emp.name}</h3><p className="text-xs uppercase">{emp.status === 'in' ? 'Clocked In' : 'Away'}</p></div>
              </div>
            </div>
          ))}
        </div>
            ...
            <div className="space-y-4 max-h-[500px] overflow-y-auto">
              {logs.slice(0, 20).map(log => (
                <div key={log.id} className="flex items-start gap-3 text-sm pb-2 border-b">
                  {log.photoUrl ? <img src={log.photoUrl} className="w-10 h-10 rounded object-cover bg-slate-100" /> : <div className="w-10 h-10 rounded bg-slate-100"/>}
                  <div className="flex-1">
                    <div className="flex justify-between">
                      <div className="font-medium">{log.employeeName} {log.action.includes('In') && isLate(log.timestamp, log.shiftStart || settings.defaultShift) && <span className="text-red-500 text-xs font-bold ml-2">LATE</span>}</div>
      ...
      {activeTab === 'reports' && (
        <div className="bg-white p-6 rounded-xl border border-slate-200">
          <h2 className="font-bold mb-4">Log Export</h2>
          <button className="bg-green-600 text-white px-4 py-2 rounded" onClick={() => {
             let csv = "Name,Date,Time,Action,Location\n";
             ...
             const link = document.createElement("a"); link.href = encodeURI("data:text/csv;charset=utf-8," + csv); link.download = "logs.csv"; link.click();
          }}>Download CSV</button>
        </div>
      )}
      ...
      {activeTab === 'settings' && (
        <div className="max-w-md mx-auto bg-white p-6 rounded-xl border border-slate-200 space-y-4">
          ...
          <button onClick={async () => { if(db) { 
            await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'config'), settings); alert('Saved'); 
          } }} className="w-full bg-slate-900 text-white py-2 rounded">Save Config</button>
        </div>
      )}
    </div>
  );
...
function KioskMode({ userId, kioskLocation }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (prevStatus === 'locked') {
        if (emp.isKeyholder && emp.codeValidDate === today) {
           await performAction(empDoc, emp, 'Clock In (Unlock)', 'in', photo);
           setStatus('success'); setMessage(`Unlocked: ${emp.name}`);
           setTimeout(() => { setStatus('idle'); setInputCode(''); }, 2000);
        } else {
           setStatus('error'); setMessage('Auth Failed'); setTimeout(() => { setStatus('locked'); setInputCode(''); }, 2000);
        }
        return;
      }
      ...
      setStatus('success'); setMessage(emp.status === 'in' ? 'Goodbye!' : dailyMsg || 'Welcome!');
  ...
  const performAction = async (ref, data, action, newStatus, photo) => {
    await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'logs'), {
      employeeId: ref.id, employeeName: data.name, shiftStart: data.shiftStart || '08:00', action, timestamp: serverTimestamp(), photoUrl: photo, location: kioskLocation
    });
    ...
    <div className={`min-h-full flex flex-col items-center justify-center p-4 transition-colors duration-500 ${status === 'locked' ? 'bg-black' : 'bg-slate-900'} relative overflow-hidden flex-1`}>
      ...
      <div className={`relative z-10 bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden ${status === 'locked' ? 'opacity-80' : 'opacity-100'}`}>
        <div className={`h-32 flex flex-col items-center justify-center p-6 ${status === 'success' ? 'bg-green-500' : status === 'error' ? 'bg-red-500' : status === 'locked' ? 'bg-slate-800' : 'bg-slate-100'}`}>
          ...
          {(status === 'locked' || status === 'idle' || status === 'processing') && (
             <>
               <div className={`text-sm mb-2 font-medium flex items-center gap-2 ${status === 'locked' ? 'text-slate-400' : 'text-slate-500'}`}>{status === 'locked' ? <><Key size={14}/> KEYHOLDER UNLOCK</> : <><Camera size={14}/> ENTER CODE</>}</div>
               <div className={`text-4xl font-mono font-bold tracking-[0.5em] h-10 ${status === 'locked' ? 'text-white' : 'text-slate-800'}`}>{inputCode.padEnd(6, '•')}</div>
             </>
          )}
        </div>
